...
s32 Math3D_LineVsLineClosestTwoPoints(Vec3f* lineAPointA, Vec3f* lineAPointB,
                                      Vec3f* lineBPointA, Vec3f* lineBPointB,
                                      Vec3f* lineAClosestToB,
                                      Vec3f* lineBClosestToA);
s32 Math3D_TriLineIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny,
                            f32 nz, f32 originDist, Vec3f* linePointA,
                            Vec3f* linePointB, Vec3f* intersect, s32 fromFront);
s32 Math3D_PlaneVsPlaneNewLine(f32 planeAA, f32 planeAB, f32 planeAC,
                               f32 planeADist, f32 planeBA, f32 planeBB,
                               f32 planeBC, f32 planeBDist,
                               InfiniteLine* intersect);
s32 Math3D_CirSquareVsTriSquare(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2,
                                f32 centerX, f32 centerY, f32 radius);
s32 Math3D_SphCubeVsTriCube(Vec3f* v0, Vec3f* v1, Vec3f* v2, Vec3f* center,
                            f32 radius);

/**
 * Creates an infinite line along the intersection of the plane defined from
 * `planeAA`x + `planeAB`y + `planeAB`z + `planeADist` = 0 and `planeBA`x +
 * `planeBB`y + `planeBC`z + `planeBDist` = 0, and finds the closest point on
 * that intersection to the line segment `linePointA and linePointB`, outputs
 * the intersection to `closestPoint`
 */
s32 Math3D_PlaneVsLineSegClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC,
                                      f32 planeADist, f32 planeBA, f32 planeBB,
                                      f32 planeBC, f32 planeBDist,
                                      Vec3f* linePointA, Vec3f* linePointB,
                                      Vec3f* closestPoint) {
  ...
  planeIntersectSeg.b.x =
      (planeIntersectLine.dir.x * 100.0f) + planeIntersectLine.point.x;
  planeIntersectSeg.b.y =
      (planeIntersectLine.dir.y * 100.0f) + planeIntersectLine.point.y;
  planeIntersectSeg.b.z =
      (planeIntersectLine.dir.z * 100.0f) + planeIntersectLine.point.z;

  // closestPoint is a point on planeIntersect, sp34 is a point on linePointA,
  // linePointB
  if (!Math3D_LineVsLineClosestTwoPoints(&planeIntersectSeg.a,
                                         &planeIntersectSeg.b, linePointA,
                                         linePointB, closestPoint, &sp34)) {
    return false;
  }
  return true;
}
...
/**
 * Determines the closest point on the line `line` to `pos`, by forming a line
 * perpendicular from `point` to `line` closest point is placed in
 * `closestPoint`
 */
void Math3D_LineClosestToPoint(InfiniteLine* line, Vec3f* pos,
                               Vec3f* closestPoint) {
  ...
  t = (((pos->x - line->point.x) * line->dir.x) +
       ((pos->y - line->point.y) * line->dir.y) +
...
void Math3D_FindPointOnPlaneIntersect(f32 planeAAxis1Norm, f32 planeAAxis2Norm,
                                      f32 planeBAxis1Norm, f32 planeBAxis2Norm,
                                      f32 axis3Direction, f32 planeADist,
                                      f32 planeBDist, f32* axis1Point,
                                      f32* axis2Point) {
  *axis1Point =
...
/**
 * Creates a line between the intersections of two planes defined from
 * `planeAA`x + `planeAB`y + `planeAC`z + `planeADist` = 0 and `planeBA`x +
 * `planeBB`y + `planeBC`z + `planeBDist` = 0, and outputs the line to
 * `intersect`. Returns false if the planes are parallel.
 */
s32 Math3D_PlaneVsPlaneNewLine(f32 planeAA, f32 planeAB, f32 planeAC,
                               f32 planeADist, f32 planeBA, f32 planeBB,
                               f32 planeBC, f32 planeBDist,
  ...
  if (IS_ZERO(intersect->dir.x) && IS_ZERO(intersect->dir.y) &&
      IS_ZERO(intersect->dir.z)) {
    // planes are parallel
    return false;
  }
  ...
  if ((dirX >= dirY) && (dirX >= dirZ)) {
    Math3D_FindPointOnPlaneIntersect(planeAB, planeAC, planeBB, planeBC,
                                     intersect->dir.x, planeADist, planeBDist,
    ...
    Math3D_FindPointOnPlaneIntersect(planeAC, planeAA, planeBC, planeBA,
                                     intersect->dir.y, planeADist, planeBDist,
    ...
    Math3D_FindPointOnPlaneIntersect(planeAA, planeAB, planeBA, planeBB,
                                     intersect->dir.z, planeADist, planeBDist,
...
/**
 * Gets the closest point on the line formed from the intersection of of the
 * planes defined from `planeAA`x + `planeAB`y + `planeAC`z + `planeADist` = 0
 * and `planeBA`x + `planeBB`y + `planeBC`z + `planeBDist` = 0 the point on the
 * intersection line closest to `point` is placed in `closestPoint` returns
 * false if the planes are parallel.
 */
s32 Math3D_PlaneVsPlaneVsLineClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC,
                                          f32 planeADist, f32 planeBA,
                                          f32 planeBB, f32 planeBC,
                                          f32 planeBDist, Vec3f* point,
                                          Vec3f* closestPoint) {
...
/**
 * Finds a point on the line from starting point `v0`, and directional vector
 * `dir` which is `dist` length from the starting point.  Result is placed in
 * `ret`
 */
...
/**
 * Splits the line segment from end points `v0` and `v1`, and splits that
 * segment by `ratio` of `v0`:`v1`, places the resulting point on the line in
 * `ret`
 */
...
/**
 * Calculates the cosine between vectors `a` and `b` and places the result in
 * `ret` returns true if the cosine cannot be calculated because the product of
 * the magnitudes is zero
 */
...
/**
 * Reflects vector `vec` across the normal vector `normal`, reflection vector is
 * placed in `reflVec`
 */
...
/**
 * Checks if the point (`x`,`y`) is contained within the square formed from
 * (`upperLeftX`,`upperLeftY`) to
 * (`lowerRightX`,`lowerRightY`)
 */
s32 Math3D_PointInSquare2D(f32 upperLeftX, f32 lowerRightX, f32 upperLeftY,
                           f32 lowerRightY, f32 x, f32 y) {
...
/**
 * Checks if the square formed around the circle with center
 * (`centerX`,`centerY`) with radius `radius` touches any portion of the square
 * formed around the triangle with vertices (`x0`,`y0`), (`x1`,`y1`), and
 * (`x2`,`y2`)
 */
s32 Math3D_CirSquareVsTriSquare(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2,
                                f32 centerX, f32 centerY, f32 radius) {
...
/**
 * Checks if the cube formed around the triangle formed from `v0`, `v1`, and
 * `v2` has any portion touching the cube formed around the sphere with center
 * `center` and radius of `radius`
 */
s32 Math3D_SphCubeVsTriCube(Vec3f* v0, Vec3f* v1, Vec3f* v2, Vec3f* center,
                            f32 radius) {
  f32 minX;
 ...
 */
f32 Math3D_Dist1DSq(f32 a, f32 b) { return SQ(a) + SQ(b); }

/**
 * Returns the distance between `a` and `b` on a single axis
 */
f32 Math3D_Dist1D(f32 a, f32 b) { return sqrtf(Math3D_Dist1DSq(a, b)); }
...
/**
 * Gets the Z portion of the cross product of vectors `a - (`dx`,`dy`,z) and `b`
 * - (`dx`,`dy`,z)
 */
...
/**
 * Gets the X portion of the cross product of vectors `a - (x,`dy`,`dz`) and `b`
 * - (x,`dy`,`dz`)
 */
...
/**
 * Gets the Y portion of the cross product of vectors `a - (`dx`,y,`dz`) and `b`
 * - (`dx`,y,`dz`)
 */
...
/*
 * Calculates the normal vector to a surface with sides `vb` - `va` and `vc` -
 * `va` outputs the normal to `normal`
 */
...
s32 Math3D_LineVsCube(Vec3f* min, Vec3f* max, Vec3f* a, Vec3f* b) {
  ...
  triVtx2.y = max->y;
  triVtx2.z = max->z;
  if (Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, -1.0f, 0.0f, 0.0f,
                              min->x, a, b, &intersectPoint, 0)) {
    return true;
  }
  ...
  triVtx2.y = max->y;
  triVtx2.z = max->z;
  if (Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, 0.0f, 1.0f,
                              -max->z, a, b, &intersectPoint, 0)) {
    return true;
  }
  triVtx0.x = max->x;
  triVtx0.y = max->y;
  ...
  triVtx1.z = max->z;
  triVtx2.x = max->x;
  //! @bug trVtx1.y should be triVtx2.y, prevents a tri on the cube from being
  //! checked.
  triVtx1.y = min->y;
  triVtx2.z = max->z;
  if (Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, 0.0f, 1.0f,
  ...
  triVtx2.y = max->y;
  triVtx2.z = max->z;
  if (Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, 1.0f, 0.0f,
                              -max->y, a, b, &intersectPoint, 0)) {
    return true;
  }
  triVtx0.x = max->x;
  triVtx0.y = max->y;
 ...
 * outputs the plane equation `a``pointOnPlane->x` + 0y +
 * `c``pointOnPlane->z`+`d` = 0
 */
void Math3D_RotateXZPlane(Vec3f* pointOnPlane, s16 angle, f32* a, f32* c,
                          f32* d) {
...
/*
 * Defines a plane from vertices `va`, `vb`, and `vc`.  Normal components are
 * output to `nx`, `ny`, and `nz`.  Distance from the origin is output to
 * `originDist` Satisfies the plane equation NxVx + NyVy + NzVz + D = 0
 */
void Math3D_DefPlane(Vec3f* va, Vec3f* vb, Vec3f* vc, f32* nx, f32* ny, f32* nz,
                     f32* originDist) {
  static Vec3f normal;
...
/*
 * Returns the answer to the plane equation with elements specified by
 * arguments.
 */
...
/**
 * Checks if the point defined by (`z`,`x`) is within distance of the triangle
 * defined from `v0`,`v1`, and `v2`
 */
s32 Math3D_TriChkPointParaYImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z, f32 x,
                                f32 detMax, f32 chkDist, f32 ny) {
  f32 detv0v1;
  f32 detv1v2;
  f32 detv2v0;
  f32 distToEdgeSq;
  ...
  if (!Math3D_CirSquareVsTriSquare(v0->z, v0->x, v1->z, v1->x, v2->z, v2->x, z,
                                   x, chkDist)) {
    return false;
  }
  ...
  if (((SQ(v0->z - z) + SQ(v0->x - x)) < chkDistSq) ||
      ((SQ(v1->z - z) + SQ(v1->x - x)) < chkDistSq) ||
  ...
  if (fabsf(ny) > 0.5f) {
    // Do a check on each face of the triangle, if the point is within `chkDist`
    // units return true.
    if (Math3D_PointDistSqToLine2D(z, x, v0->z, v0->x, v1->z, v1->x,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }

    if (Math3D_PointDistSqToLine2D(z, x, v1->z, v1->x, v2->z, v2->x,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }

    if (Math3D_PointDistSqToLine2D(z, x, v2->z, v2->x, v0->z, v0->x,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }
...
s32 Math3D_TriChkPointParaYDeterminate(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z,
                                       f32 x, f32 detMax, f32 ny) {
  return Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, detMax, 1.0f, ny);
}

s32 Math3D_TriChkPointParaYSlopedY(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z,
                                   f32 x) {
  return Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 300.0f, 1.0f, 0.6f);
}

/**
 * Performs the triangle and point check parallel to the Y axis, outputs the y
 * coordinate of the point to `yIntersect`
 */
s32 Math3D_TriChkPointParaYIntersectDist(Vec3f* v0, Vec3f* v1, Vec3f* v2,
                                         f32 nx, f32 ny, f32 nz, f32 originDist,
                                         f32 z, f32 x, f32* yIntersect,
                                         f32 chkDist) {
  if (IS_ZERO(ny)) {
    return false;
  }
...
s32 Math3D_TriChkPointParaYIntersectInsideTri(Vec3f* v0, Vec3f* v1, Vec3f* v2,
                                              f32 nx, f32 ny, f32 nz,
                                              f32 originDist, f32 z, f32 x,
...
s32 Math3D_TriChkPointParaY(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 ny, f32 z,
                            f32 x) {
  if (IS_ZERO(ny)) {
    return false;
  }
...
s32 Math3D_TriChkLineSegParaYIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx,
                                       f32 ny, f32 nz, f32 originDist, f32 z,
  ...
  if (((pointADist > 0.0f) && (pointBDist > 0.0f)) ||
      ((pointADist < 0.0f) && (pointBDist < 0.0f))) {
    return false;
  }
...
s32 Math3D_TriChkPointParaYDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane,
                                f32 z, f32 x, f32 chkDist) {
  if (IS_ZERO(plane->normal.y)) {
    return false;
  }

  if (Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 0.0f, chkDist,
                                  plane->normal.y)) {
    return true;
  }
...
s32 Math3D_TriChkPointParaXImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 y, f32 z,
                                f32 detMax, f32 chkDist, f32 nx) {
  f32 detv0v1;
  f32 detv1v2;
  f32 detv2v0;
  f32 distToEdgeSq;
  ...
  if (!Math3D_CirSquareVsTriSquare(v0->y, v0->z, v1->y, v1->z, v2->y, v2->z, y,
                                   z, chkDist)) {
    return false;
  }
  ...
  if (((SQ(v0->y - y) + SQ(v0->z - z)) < chkDistSq) ||
  ...
  if (fabsf(nx) > 0.5f) {
    if (Math3D_PointDistSqToLine2D(y, z, v0->y, v0->z, v1->y, v1->z,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }

    if (Math3D_PointDistSqToLine2D(y, z, v1->y, v1->z, v2->y, v2->z,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }

    if (Math3D_PointDistSqToLine2D(y, z, v2->y, v2->z, v0->y, v0->z,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }
...
s32 Math3D_TriChkPointParaXDeterminate(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 y,
                                       f32 z, f32 detMax, f32 nx) {
  return Math3D_TriChkPointParaXImpl(v0, v1, v2, y, z, detMax, 1.0f, nx);
}

s32 Math3D_TriChkPointParaXIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx,
                                     f32 ny, f32 nz, f32 originDist, f32 y,
...
s32 Math3D_TriChkPointParaX(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 y,
                            f32 z) {
  if (IS_ZERO(nx)) {
    return false;
  }
...
s32 Math3D_TriChkLineSegParaXIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx,
                                       f32 ny, f32 nz, f32 originDist, f32 y,
...
s32 Math3D_TriChkPointParaXDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane,
                                f32 y, f32 z, f32 chkDist) {
  if (IS_ZERO(plane->normal.x)) {
    return false;
  }
  if (Math3D_TriChkPointParaXImpl(v0, v1, v2, y, z, 0.0f, chkDist,
                                  plane->normal.x)) {
    return true;
  }
  return false;
...
s32 Math3D_TriChkPointParaZImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 x, f32 y,
                                f32 detMax, f32 chkDist, f32 nz) {
  f32 detv0v1;
  f32 detv1v2;
  f32 detv2v0;
  f32 distToEdgeSq;
  ...
  if (!Math3D_CirSquareVsTriSquare(v0->x, v0->y, v1->x, v1->y, v2->x, v2->y, x,
                                   y, chkDist)) {
    return false;
  }
  ...
  if (((SQ(x - v0->x) + SQ(y - v0->y)) < chkDistSq) ||
      ((SQ(x - v1->x) + SQ(y - v1->y)) < chkDistSq) ||
  ...
  if (fabsf(nz) > 0.5f) {
    if (Math3D_PointDistSqToLine2D(x, y, v0->x, v0->y, v1->x, v1->y,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }

    if (Math3D_PointDistSqToLine2D(x, y, v1->x, v1->y, v2->x, v2->y,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }

    if (Math3D_PointDistSqToLine2D(x, y, v2->x, v2->y, v0->x, v0->y,
                                   &distToEdgeSq) &&
        (distToEdgeSq < chkDistSq)) {
      return true;
    }
...
s32 Math3D_TriChkPointParaZDeterminate(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 x,
                                       f32 y, f32 detMax, f32 nz) {
  return Math3D_TriChkPointParaZImpl(v0, v1, v2, x, y, detMax, 1.0f, nz);
}

s32 Math3D_TriChkPointParaZIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx,
                                     f32 ny, f32 nz, f32 originDist, f32 x,
...
s32 Math3D_TriChkPointParaZ(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nz, f32 x,
                            f32 y) {
  if (IS_ZERO(nz)) {
    return false;
  }
...
s32 Math3D_TriChkLineSegParaZIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx,
                                       f32 ny, f32 nz, f32 originDist, f32 x,
...
s32 Math3D_TriChkLineSegParaZDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane,
                                  f32 x, f32 y, f32 chkDist) {
  if (IS_ZERO(plane->normal.z)) {
    return false;
  }
  if (Math3D_TriChkPointParaZImpl(v0, v1, v2, x, y, 0.0f, chkDist,
                                  plane->normal.z)) {
    return true;
  }
  return false;
...
s32 Math3D_LineSegFindPlaneIntersect(f32 pointADist, f32 pointBDist,
                                     Vec3f* pointA, Vec3f* pointB,
                                     Vec3f* intersect) {
  f32 distDiff;
...
/**
 * Determines if the line segment from `linePointA` to `linePointB` crosses the
 * plane from `nx` + `ny` + `nz` + `originDist` = 0.  If fromFront is set, then
 * detection will only be true if point A crosses from the front of the plane
 */
s32 Math3D_LineSegVsPlane(f32 nx, f32 ny, f32 nz, f32 originDist,
                          Vec3f* linePointA, Vec3f* linePointB,
  ...
  return Math3D_LineSegFindPlaneIntersect(pointADist, pointBDist, linePointA,
                                          linePointB, intersect);
}

/*
 * Determines if the line formed by `linePointA` and `linePointB` intersect with
 * Triangle formed from vertices `v0`, `v1`, and `v2` with normal vector `nx`,
 * `ny`, and `nz` with plane distance from origin `originDist` Outputs the
 * intersection point at to `intersect` Returns 1 if the line intersects with
 * the triangle, 0 otherwise
 */
s32 Math3D_TriLineIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny,
                            f32 nz, f32 originDist, Vec3f* linePointA,
                            Vec3f* linePointB, Vec3f* intersect,
                            s32 fromFront) {
  ...
  if (((nx == 0.0f) ||
       (Math3D_TriChkPointParaX(v0, v1, v2, nx, intersect->y, intersect->z))) &&
      ((ny == 0.0f) ||
       (Math3D_TriChkPointParaY(v0, v1, v2, ny, intersect->z, intersect->x))) &&
      ((nz == 0.0f) ||
       (Math3D_TriChkPointParaZ(v0, v1, v2, nz, intersect->x, intersect->y)))) {
    return true;
  }
...
/*
 * Creates a TriNorm output to `tri`, and calculates the normal vector and plane
 * from vertices `va`, `vb`, and `vc`
 */
void Math3D_TriNorm(TriNorm* tri, Vec3f* va, Vec3f* vb, Vec3f* vc) {
  tri->vtx[0] = *va;
  tri->vtx[1] = *vb;
  tri->vtx[2] = *vc;
  Math3D_DefPlane(va, vb, vc, &tri->plane.normal.x, &tri->plane.normal.y,
...
/**
 * Determines the distance from point (`x0`,`y0`) to the line formed from
 * (`x1`,`y1`) and (`x2`,`y2`) Distance squared is output to `lineLenSq`,
 * returns true if the point perpendicular from (`x0`,`y0`) is contained within
 * the segment between (`x1`,`y1`) and (`x2`,`y2`)
 */
s32 Math3D_PointDistSqToLine2D(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2,
                               f32* lineLenSq) {
...
/**
 * Determines if the line `line` is touching the sphere `sphere` at any point in
 * the line.
 */
s32 Math3D_LineVsSph(Sphere16* sphere, Linef* line) {
  ...
  if ((Math3D_PointInSph(sphere, &line->a)) ||
      (Math3D_PointInSph(sphere, &line->b))) {
  ...
  temp_f0_2 = ((((sphere->center.x - line->a.x) * lineDiff.x) +
                ((sphere->center.y - line->a.y) * lineDiff.y)) +
  ...
  if (SQ(sphLinePerpendicularPoint.x - sphere->center.x) +
          SQ(sphLinePerpendicularPoint.y - sphere->center.y) +
...
/**
 * Gets the surface point of `sphere` intersecting with `tri` generated from the
 * line formed from the sphere's surface to the midpoint of the line formed from
 * the first two vertices of the tri
 */
void Math3D_GetSphVsTriIntersectPoint(Sphere16* sphere, TriNorm* tri,
  ...
  // Distance from the sphere's center to the center of the line formed from
  // v0->v1
  if (IS_ZERO(dist)) {
...
/**
 * Determines if `sphere` and `tri` and touching, and outputs the intersection
 * point to `intersectPoint`
 */
s32 Math3D_TriVsSphIntersect(Sphere16* sphere, TriNorm* tri,
                             Vec3f* intersectPoint) {
  ...
  if (!Math3D_SphCubeVsTriCube(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2],
                               &sphereCenter, radius)) {
    return false;
  }

  planeDist = Math3D_UDistPlaneToPos(tri->plane.normal.x, tri->plane.normal.y,
                                     tri->plane.normal.z, tri->plane.originDist,
                                     &sphereCenter);
  if (radius < planeDist) {
    // the point that lies within the plane of the triangle which is
    // perpendicular to the sphere's center is more than the radius of the
    // sphere, the plane never crosses the sphere.
    return false;
  }
  ...
  if (fabsf(tri->plane.normal.y) > 0.5f) {
    if (Math3D_TriChkPointParaYDeterminate(
            &tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.z,
            sphPlanePos.x, 0.0f, tri->plane.normal.y)) {
      Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
      return true;
    }
  } else if (fabsf(tri->plane.normal.x) > 0.5f) {
    if (Math3D_TriChkPointParaXDeterminate(
            &tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.y,
            sphPlanePos.z, 0.0f, tri->plane.normal.x)) {
      Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
      return true;
    }
  } else if (Math3D_TriChkPointParaZDeterminate(
                 &tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.x,
...
s32 Math3D_PointInCyl(Cylinder16* cyl, Vec3f* point) {
  ...
  if ((SQ(x) + SQ(z)) < SQ(cyl->radius) && (bottom < point->y) &&
      (point->y < top)) {
    return true;
  } else {
    return false;
...
s32 Math3D_CylVsLineSeg(Cylinder16* cyl, Vec3f* linePointA, Vec3f* linePointB,
                        Vec3f* intersectA, Vec3f* intersectB) {
  Vec3f cylToPtA;
  Vec3f cylToPtB;
  Vec3f ptAToPtB;
  f32 fracA;
  ...
  if (Math3D_PointInCyl(cyl, linePointA) &&
      Math3D_PointInCyl(cyl, linePointB)) {
  ...
  /**
   * This section finds the points of intersection of the infinite line
   * containing AB with the side of the infinite cylinder containing cyl.
   * Intersection points beyond the bounds of the segment and cylinder are
  ...
  if (!IS_ZERO(2.0f * SQXZ(ptAToPtB))) {
    ...
    // Used if the line segment is nearly vertical. Unclear what it's
    // calculating.
  ...
  if ((sideIntA == 1) && ((fracA * ptAToPtB.y + cylToPtA.y) < 0.0f ||
                          cyl->height < (fracA * ptAToPtB.y + cylToPtA.y))) {
    sideIntA = 0;
  }
  if ((sideIntB == 1) && ((fracB * ptAToPtB.y + cylToPtA.y) < 0.0f ||
                          cyl->height < (fracB * ptAToPtB.y + cylToPtA.y))) {
    sideIntB = 0;
  }
  ...
  /**
   * Places the found intersection points into intersectA and intersectB.
   * IntersectA is always closer to point A
   */
      ...
      if (count == 0) {
        *intersectA = intPts[i];
        ...
        if (Math3D_Vec3fDistSq(intersectA, linePointA) <
            Math3D_Vec3fDistSq(intersectA, &intPts[i])) {
          *intersectB = intPts[i];
        } else {
...
s32 Math3D_CylTriVsIntersect(Cylinder16* cyl, TriNorm* tri, Vec3f* intersect) {
  ...
  if (((tri->vtx[0].y < cylBottom) && (tri->vtx[1].y < cylBottom) &&
       (tri->vtx[2].y < cylBottom)) ||
      ((cylTop < tri->vtx[0].y) && (cylTop < tri->vtx[1].y) &&
       (cylTop < tri->vtx[2].y))) {
    // If all of the vertices are below or all of the vertices are above the
    // cylinder.
    return false;
  }

  minDistSq = 1.e38f;
  if (Math3D_CylVsLineSeg(cyl, &tri->vtx[0], &tri->vtx[1], &cylIntersectA,
                          &cylIntersectB)) {
    distFromPointAToIntersectASq =
        Math3D_Vec3fDistSq(&cylIntersectA, &tri->vtx[0]);
    minDistSq = distFromPointAToIntersectASq;
    *intersect = cylIntersectA;
  }

  if (Math3D_CylVsLineSeg(cyl, &tri->vtx[2], &tri->vtx[1], &cylIntersectA,
                          &cylIntersectB)) {
    distFromPointAToIntersectASq =
        Math3D_Vec3fDistSq(&cylIntersectA, &tri->vtx[2]);
  ...
  if (Math3D_CylVsLineSeg(cyl, &tri->vtx[0], &tri->vtx[2], &cylIntersectA,
                          &cylIntersectB)) {
    distFromPointAToIntersectASq =
        Math3D_Vec3fDistSq(&cylIntersectA, &tri->vtx[0]);
    if (distFromPointAToIntersectASq < minDistSq) {
  ...
  if (Math3D_TriChkLineSegParaYIntersect(
          &tri->vtx[0], &tri->vtx[1], &tri->vtx[2], tri->plane.normal.x,
    ...
    distFromCylYIntersectTov0v1 =
        sqrtf(SQ(diffMidpointIntersect.x) + SQ(diffMidpointIntersect.z));
    ...
    radiusTodistFromCylYIntersectTov0v1 =
        cyl->radius / distFromCylYIntersectTov0v1;
    Math3D_PointOnInfiniteLine(&cylIntersectCenter, &diffMidpointIntersect,
                               radiusTodistFromCylYIntersectTov0v1, intersect);
    return true;
  }
...
/*
 * Determines if two spheres are touching.  The amount that they're overlapping
 * is placed in `overlapSize`
 */
s32 Math3D_SphVsSphOverlap(Sphere16* sphereA, Sphere16* sphereB,
                           f32* overlapSize) {
  f32 centerDist;

  return Math3D_SphVsSphOverlapCenterDist(sphereA, sphereB, overlapSize,
                                          &centerDist);
}

/*
 * Determines if two spheres are touching  The distance from the centers is
 * placed in `centerDist`, and the amount that they're overlapping is placed in
 * `overlapSize`
 */
s32 Math3D_SphVsSphOverlapCenterDist(Sphere16* sphereA, Sphere16* sphereB,
                                     f32* overlapSize, f32* centerDist) {
  Vec3f diff;
...
/**
 * Checks if `sph` and `cyl` are touching, output the amount of xz overlap to
 * `overlapSize`
 */
...
/**
 * Checks if `sph` and `cyl` are touching, output the xz distance of the centers
 * to `centerDist`, and the amount of xz overlap to `overlapSize`
 */
s32 Math3D_SphVsCylOverlapCenterDist(Sphere16* sph, Cylinder16* cyl,
                                     f32* overlapSize, f32* centerDist) {
  ...
  *centerDist = sqrtf(SQ(x) + SQ(z));
  if (combinedRadius < *centerDist) {
    // if the combined radii is less than the distance to the centers, they
    // cannot be touching.
    return false;
  }
  ...
  if ((sphTop >= cylBottom) && (sphBottom <= cylTop)) {
    // if the cylinder and sphere are intersecting on the xz plane, check if
    // they're intersecting on the y axis.
...
/**
 * Checks if `ca` and `cb` are touching, output the amount of xz overlap to
 * `overlapSize`
 */
...
/**
 * Checks if `ca` and `cb` are touching, output the xz distance of the centers
 * to `centerDist`, and the amount of xz overlap to `overlapSize`
 */
s32 Math3D_CylVsCylOverlapCenterDist(Cylinder16* ca, Cylinder16* cb,
                                     f32* overlapSize, f32* centerDist) {
...
s32 Math3D_TriVsTriIntersect(TriNorm* ta, TriNorm* tb, Vec3f* intersect) {
  ...
  if (Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2],
                              tb->plane.normal.x, tb->plane.normal.y,
                              tb->plane.normal.z, tb->plane.originDist,
                              &ta->vtx[0], &ta->vtx[1], intersect, 0)) {
    return true;
  }
  if (Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2],
                              tb->plane.normal.x, tb->plane.normal.y,
                              tb->plane.normal.z, tb->plane.originDist,
                              &ta->vtx[1], &ta->vtx[2], intersect, 0)) {
    return true;
  }
  if (Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2],
                              tb->plane.normal.x, tb->plane.normal.y,
                              tb->plane.normal.z, tb->plane.originDist,
                              &ta->vtx[2], &ta->vtx[0], intersect, 0)) {
    return true;
  }
  if (Math3D_TriLineIntersect(&ta->vtx[0], &ta->vtx[1], &ta->vtx[2],
                              ta->plane.normal.x, ta->plane.normal.y,
                              ta->plane.normal.z, ta->plane.originDist,
                              &tb->vtx[0], &tb->vtx[1], intersect, 0) == 1) {
    return true;
  }
  if (Math3D_TriLineIntersect(&ta->vtx[0], &ta->vtx[1], &ta->vtx[2],
                              ta->plane.normal.x, ta->plane.normal.y,
                              ta->plane.normal.z, ta->plane.originDist,
                              &tb->vtx[1], &tb->vtx[2], intersect, 0) == 1) {
    return true;
  }
  if (Math3D_TriLineIntersect(&ta->vtx[0], &ta->vtx[1], &ta->vtx[2],
                              ta->plane.normal.x, ta->plane.normal.y,
                              ta->plane.normal.z, ta->plane.originDist,
                              &tb->vtx[2], &tb->vtx[0], intersect, 0) == 1) {
    return true;
  }
  return false;
}
