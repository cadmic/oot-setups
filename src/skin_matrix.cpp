...
/**
 * Multiplies the matrix mf by a 4 components column vector [ src , 1 ] and
 * writes the resulting 4 components to xyzDest and wDest.
 *
 ...
 * \f]
 */
void SkinMatrix_Vec3fMtxFMultXYZW(MtxF* mf, Vec3f* src, Vec3f* xyzDest,
                                  f32* wDest) {
  xyzDest->x =
...
/**
 * Multiplies the matrix mf by a 4 components column vector [ src , 1 ] and
 * writes the resulting xyz components to dest.
 *
 ...
 */
void SkinMatrix_GetClear(MtxF** mfp) { *mfp = &sMtxFClear; }
...
/**
 * Produces a matrix which scales x,y,z components of vectors or x,y,z rows of
 * matrices (when applied on LHS)
 */
...
/**
 * Produces a matrix which translates a vector by amounts in the x, y and z
 * directions
 */
...
/**
 * Produces a matrix which scales, then rotates (using ZYX Tait-Bryan angles),
 * then translates.
 */
void SkinMatrix_SetTranslateRotateZYXScale(MtxF* dest, f32 scaleX, f32 scaleY,
                                           f32 scaleZ, s16 rotX, s16 rotY,
                                           s16 rotZ, f32 translateX,
...
/**
 * Produces a matrix which scales, then rotates (using YXZ Tait-Bryan angles),
 * then translates.
 */
void SkinMatrix_SetTranslateRotateYXZScale(MtxF* dest, f32 scaleX, f32 scaleY,
                                           f32 scaleZ, s16 rotX, s16 rotY,
                                           s16 rotZ, f32 translateX,
                                           f32 translateY, f32 translateZ) {
  MtxF mft1;
...
/**
 * Produces a matrix which rotates (using ZYX Tait-Bryan angles), then
 * translates.
 */
void SkinMatrix_SetTranslateRotateZYX(MtxF* dest, s16 rotX, s16 rotY, s16 rotZ,
                                      f32 translateX, f32 translateY,
                                      f32 translateZ) {
  MtxF rotation;
  MtxF translation;
...
/**
 * Produces a matrix which rotates by binary angle `angle` around a unit vector
 * (`axisX`,`axisY`,`axisZ`). NB: the rotation axis is assumed to be a unit
 * vector.
 */
void SkinMatrix_SetRotateAxis(MtxF* mf, s16 angle, f32 axisX, f32 axisY,
                              f32 axisZ) {
  f32 sinA;
  f32 cosA;
  f32 xx;
...
void func_800A8030(MtxF* mf, f32* arg1) {
  ...
  n = 2.0f /
      ((arg1[3] * arg1[3]) +
       ((arg1[2] * arg1[2]) + ((arg1[1] * arg1[1]) + (arg1[0] * arg1[0]))));
  xNorm = arg1[0] * n;
  yNorm = arg1[1] * n;
  zNorm = arg1[2] * n;
